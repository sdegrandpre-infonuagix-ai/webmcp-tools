...
export async function runEvaluations(
  ...
  try {
    ...
    if ("url" in config) {
      ...
      const tests: Array<Eval> = JSON.parse(await readFile(config.evalsFile, "utf-8"));
      ...
      const toolsSchema: ToolsSchema = JSON.parse(await readFile(config.toolSchemaFile, "utf-8"));
